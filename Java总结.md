...
#### 7.**在浏览器地址栏输入一个url到浏览器返回页面的过程？** <重点>
...
> 
>
> - **LFU(Least Frequently Used)算法**：即最不经常使用页置换算法，要求在页置换时置换==引用计数==最小的页，因为经常使用的页应该有一个较大的引用次数。**与页面使用次数有关**。 <重点>
...
#### 13.AQS原理分析（AbstractQueuedSynchronizer） <重点>
...
#### 17.Future和FutureTask的区别？==(Future等结果，FutureTask可以被提交)== <重点>
...
#### 13.先行发生原则(Happen-Before)（可见性保证） <重点>
...
### 11.**ApplicationContext通常的实现是什么?** <2>
...
### 19.Nginx 反向代理实现高并发的具体步骤是什么？ <Nginx>面试会问到
...
### 6.Redis事务？（REmote DIctionary Server(*Redis*)）(不是很重要)
...
### 1.数据库引擎？（InnoDB和MyISAM区别）(1,2,3,4,6)

| 编号 | 区别     | InnoDB (常用)                                                   | MyISAM                                                       |
| ---- | -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
